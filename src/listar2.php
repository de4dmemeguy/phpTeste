...
if (!empty($_GET['nome'])) {
    $nome = mysqli_real_escape_string($conexao, $_GET['nome']);
    $result_pessoa = "SELECT IDPESSOA, NOME, CPF, TELEFONE, DOCUM_CIVIL, ENCAMINHAMENTOS FROM pessoa WHERE NOME LIKE '%$nome%'";
} elseif (!empty($_GET['data'])) {
    $data = mysqli_real_escape_string($conexao, $_GET['data']);
    $data_formatada = date('Y-m-d', strtotime($data));
    $result_pessoa = "SELECT IDPESSOA, NOME, CPF, TELEFONE, DOCUM_CIVIL, ENCAMINHAMENTOS FROM pessoa WHERE DATE(data_hora_cadastro) ";
} else {
    $result_pessoa = "SELECT IDPESSOA, NOME, CPF, TELEFONE, DOCUM_CIVIL, ENCAMINHAMENTOS FROM pessoa";
}
        ...
        <nav aria-label="navegação da página">
                    <ul class="pagination">
                        ...
                        for ($pag_ant = $pagina - $max_links; $pag_ant <= $pagina - 1; $pag_ant++) {
                            if ($pag_ant >= 1) {
                                echo "<li class='page-item'><a class='page-link' href='listar2.php?" . (!empty($_GET['data']) ? "data=" . $_GET['data'] . "&" : "") . (!empty($_GET['nome']) ? "nome=" . $_GET['nome'] . "&" : "") ."pagina=$pag_ant'>$pag_ant </a></li>";
                            }
                        }
                        ...
                        for ($pag_dep = $pagina + 1; $pag_dep <= $pagina + $max_links; $pag_dep++) {
                            if ($pag_dep <= $quantidade_pg) {
                                echo "<li class='page-item'><a class='page-link' href='listar2.php?" . (!empty($_GET['data']) ? "data=" . $_GET['data'] . "&" : "") . (!empty($_GET['nome']) ? "nome=" . $_GET['nome'] . "&" : "") . "pagina=$pag_dep'>$pag_dep </a></li>";
                            }
                        }
                        echo "<li class='page-item'><a href='listar2.php?" . (!empty($_GET['data']) ? "data=" . $_GET['data'] . "&" : "") . (!empty($_GET['nome']) ? "nome=" . $_GET['nome'] . "&" : "") . "pagina=$quantidade_pg' class='page-link'> Ultima</a></li>";
